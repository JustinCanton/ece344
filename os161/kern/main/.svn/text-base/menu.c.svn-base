...
common_prog(int nargs, char **args)
{
	...
	result = thread_fork(args[0] /* thread name */,
			...
			cmd_progthread, &tempthread);
	if (result) {
		kprintf("thread_fork failed: %s\n", strerror(result));
		return result;
	}
	sys_waitpid(tempthread->myPid, &x, 0, &y);
	return 0;
}
...
/*
 * Print out when the dbflag command is executed
 */
...
cmd_dbflags(int n, char **a)
{
	...
	showmenu("OS/161 Debug flags", dbmenu);
...
/*
 * Debug function to determine what action needs to
 * be taken depending on the user input
 */
...
debug(int n, char **a)
{
	...
	if(!strcmp(a[1], "1") && !strcmp(a[2], "on")){	
		if((dbflags & DB_LOCORE) == DB_LOCORE){
			kprintf("DB_LOCORE is already on\n");
		}else{
			dbflags += DB_LOCORE;
		}	
	}else if(!strcmp(a[1], "2") && !strcmp(a[2], "on")){	
		if((dbflags & DB_SYSCALL) == DB_SYSCALL){
			kprintf("DB_SYSCALL is already on\n");
		}else{
			dbflags += DB_SYSCALL;
		}	
	}else if(!strcmp(a[1], "3") && !strcmp(a[2], "on")){	
		if((dbflags & DB_INTERRUPT) == DB_INTERRUPT){
			kprintf("DB_INTERRUPT is already on\n");
		}else{
			dbflags += DB_INTERRUPT;
		}	
	}else if(!strcmp(a[1], "4") && !strcmp(a[2], "on")){	
		if((dbflags & DB_DEVICE) == DB_DEVICE){
			kprintf("DB_DEVICE is already on\n");
		}else{
			dbflags += DB_DEVICE;
		}
	}else if(!strcmp(a[1], "5") && !strcmp(a[2], "on")){	
		if((dbflags & DB_THREADS) == DB_THREADS){
			kprintf("DB_THREADS is already on\n");
		}else{
			dbflags += DB_THREADS;
		}
	}else if(!strcmp(a[1], "6") && !strcmp(a[2], "on")){	
		if((dbflags & DB_VM) == DB_VM){
			kprintf("DB_VM is already on\n");
		}else{
			dbflags += DB_VM;
		}
	}else if(!strcmp(a[1], "7") && !strcmp(a[2], "on")){	
		if((dbflags & DB_EXEC) == DB_EXEC){
			kprintf("DB_EXEC is already on\n");
		}else{
			dbflags += DB_EXEC;
		}
	}else if(!strcmp(a[1], "8") && !strcmp(a[2], "on")){	
		if((dbflags & DB_VFS) == DB_VFS){
			kprintf("DB_VFS is already on\n");
		}else{
			dbflags += DB_VFS;
		}
	}else if(!strcmp(a[1], "9") && !strcmp(a[2], "on")){	
		if((dbflags & DB_SFS) == DB_SFS){
			kprintf("DB_SFS is already on\n");
		}else{
			dbflags += DB_SFS;
		}
	}else if(!strcmp(a[1], "10") && !strcmp(a[2], "on")){	
		if((dbflags & DB_NET) == DB_NET){
			kprintf("DB_NET is already on\n");
		}else{
			dbflags += DB_NET;
		}
	}else if(!strcmp(a[1], "11") && !strcmp(a[2], "on")){	
		if((dbflags & DB_NETFS) == DB_NETFS){
			kprintf("DB_NETFS is already on\n");
		}else{
			dbflags += DB_NETFS;
		}
	}else if(!strcmp(a[1], "12") && !strcmp(a[2], "on")){	
		if((dbflags & DB_KMALLOC) == DB_KMALLOC){
			kprintf("DB_KMALLOC is already on\n");
		}else{
			dbflags += DB_KMALLOC;
		}
	}else if(!strcmp(a[1], "1") && !strcmp(a[2], "off")){	
		if((dbflags & DB_LOCORE) == 0){
			kprintf("DB_LOCORE is already off\n");
		}else{
			dbflags -= DB_LOCORE;
		}	
	}else if(!strcmp(a[1], "2") && !strcmp(a[2], "off")){	
		if((dbflags & DB_SYSCALL) == 0){
			kprintf("DB_SYSCALL is already off\n");
		}else{
			dbflags -= DB_SYSCALL;
		}	
	}else if(!strcmp(a[1], "3") && !strcmp(a[2], "off")){	
		if((dbflags & DB_INTERRUPT) == 0){
			kprintf("DB_INTERRUPT is already off\n");
		}else{
			dbflags -= DB_INTERRUPT;
		}	
	}else if(!strcmp(a[1], "4") && !strcmp(a[2], "off")){	
		if((dbflags & DB_DEVICE) == 0){
			kprintf("DB_DEVICE is already off\n");
		}else{
			dbflags -= DB_DEVICE;
		}
	}else if(!strcmp(a[1], "5") && !strcmp(a[2], "off")){	
		if((dbflags & DB_THREADS) == 0){
			kprintf("DB_THREADS is already off\n");
		}else{
			dbflags -= DB_THREADS;
		}	
	}else if(!strcmp(a[1], "6") && !strcmp(a[2], "off")){	
		if((dbflags & DB_VM) == 0){
			kprintf("DB_VM is already off\n");
		}else{
			dbflags -= DB_VM;
		}
	}else if(!strcmp(a[1], "7") && !strcmp(a[2], "off")){	
		if((dbflags & DB_EXEC) == 0){
			kprintf("DB_EXEC is already off\n");
		}else{
			dbflags -= DB_EXEC;
		}
	}else if(!strcmp(a[1], "8") && !strcmp(a[2], "off")){	
		if((dbflags & DB_VFS) == 0){
			kprintf("DB_VFS is already off\n");
		}else{
			dbflags -= DB_VFS;
		}
	}else if(!strcmp(a[1], "9") && !strcmp(a[2], "off")){	
		if((dbflags & DB_SFS) == 0){
			kprintf("DB_SFS is already off\n");
		}else{
			dbflags -= DB_SFS;
		}
	}else if(!strcmp(a[1], "10") && !strcmp(a[2], "off")){	
		if((dbflags & DB_NET) == 0){
			kprintf("DB_NET is already off\n");
		}else{
			dbflags -= DB_NET;
		}
	}else if(!strcmp(a[1], "11") && !strcmp(a[2], "off")){	
		if((dbflags & DB_NETFS) == 0){
			kprintf("DB_NETFS is already off\n");
		}else{
			dbflags -= DB_NETFS;
		}	
	}else if(!strcmp(a[1], "12") && !strcmp(a[2], "off")){	
		if((dbflags & DB_KMALLOC) == 0){
			kprintf("DB_KMALLOC is already off\n");
		}else{
			dbflags -= DB_KMALLOC;
		}	
	}else{
