...
/*
 * Convenience function to cons up a uio for user I/O.
 */
void
mk_uuio(struct uio *uio, userptr_t *ubuf, size_t len, off_t pos, enum uio_rw rw)
{
	uio->uio_iovec.iov_kbase = ubuf;
	uio->uio_iovec.iov_len = len;
	uio->uio_offset = pos;
	uio->uio_resid = len;
